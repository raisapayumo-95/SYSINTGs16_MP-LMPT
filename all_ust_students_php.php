    ...
    <div class="container-fluid">
	  <div class="row">
	   ...
	   <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <h1 class="page-header"> UST Students</h1>
		...
		<form method = "post">
			<div class="form-group">
				...
				<?php 
					$student[] = array();
					$selectage = "SELECT distinct truncate(datediff(date(now()), birthday)/365.25,0) FROM university_data.student where university = 'university of santo tomas'
					order by 1";
		...
		<br>
		  <?php
			$sql = "SELECT firstname, surname, birthday, university, truncate(datediff(date(now()), birthday)/365.25,0)  FROM student where university='University of Santo Tomas'";
		  ...
		  <br>
			...
			if ( empty($_POST['student']) && (isset($_POST['asc']) || isset($_POST['desc'])))
			{
				if (isset($_POST['asc']))
				{
					...
					FROM student where university='University of Santo Tomas' ORDER BY 
			...
			else if (isset($_POST['asc']) || isset($_POST['desc']))
			{
				...
				$querymsg ="(SELECT *, truncate(datediff(date(now()), birthday)/365.25,0)  FROM student where university='University of Santo Tomas'
				...
				for ($i =0; $i < count($orderd) ; $i++)
				{
					...
					$querymsg .= $orderd[$i].") UNION
						(SELECT *, truncate(datediff(date(now()), birthday)/365.25,0)  FROM student where university='University of Santo Tomas'
						AND truncate(datediff(date(now()), birthday)/365.25,0) =";
					}
				}
				$sql = $querymsg;
			...
			{
				$sql = "SELECT firstname, surname, birthday, university, truncate(datediff(date(now()), birthday)/365.25,0)
				FROM student where university='University of Santo Tomas'";
				$result = $conn->query($sql);
			}
